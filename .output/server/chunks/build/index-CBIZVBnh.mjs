...
import { m as defu, J as kebabCase } from '../nitro/nitro.mjs';
import { n as nodeTextContent } from './node-BnIF1EG3.mjs';
export { T as TEXT_TAGS, f as flatUnwrap, i as isTag, a as isText, b as nodeChildren, u as unwrap } from './node-BnIF1EG3.mjs';
...
import { h as htmlTags } from './index-DCCfyP3w.mjs';
...
function parseHighlightedLines(lines) {
  const lineArray = String(lines || "").split(",").filter(Boolean).flatMap((line) => {
    const [start, end] = line.trim().split("-").map((a2) => Number(a2.trim()));
    return Array.from({ length: (end || start || 0) - (start || 0) + 1 }).map((_, i2) => (start || 0) + i2);
  });
...
const createParseProcessor = async (inlineOptions = {}) => {
  if (!moduleOptions) {
    moduleOptions = await import(
      './virtual_nuxt__Users_fabian_Documents_dev_araiza_node_modules_.cache_nuxt_.nuxt_mdc-imports-CbeodBy0.mjs'
      /* @vite-ignore */
    ).catch(() => ({}));
  }
  if (!generatedMdcConfigs) {
    generatedMdcConfigs = await import(
      './virtual_nuxt__Users_fabian_Documents_dev_araiza_node_modules_.cache_nuxt_.nuxt_mdc-configs-D-mPnyQ5.mjs'
      /* @vite-ignore */
...
//# sourceMappingURL=index-CBIZVBnh.mjs.map
